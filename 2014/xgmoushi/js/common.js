 ...
 * $Id: global.js 20775 2012-11-07 09:30:29Z hao.lin $
 */
		...
		var elem = options.elem,						//触发元素
			...
			(function() {
				...
				$(document.body).on('mousedown',function(e) {
					if(list.is(':visible') && !$.contains(list[0],e.target) ) {
						list.hide();
						elem.focus();
						if(callbackHide) {
...
(function () {
			...
			$("#J_login_form").ajaxForm({
				...
				success : function (data, statusText, xhr, $form) {
					...
					if (data.state === 'success') {
						...
						Wind.Util.ajaxBtnEnable(sidebar_login_btn);
						$('<div id="J_login_tips" style="display:none;"><div class="tips"><div class="tips_icon_error">'+ data.message[0] +'</div></div></div>').appendTo($('#J_sidebar_login_dt')).fadeIn(200).delay(3000).fadeOut();
					}
				}
	...
	if(head_msg_btn.length && !designing) {
		Wind.Util.clickToggle({
			elem : head_msg_btn,
			list : head_msg_pop,
			callback : function(elem, list){
	...
	if(page_wrap.length && page_wrap.data('key') && !designing) {
	...
	if (like_btn.length && GV.U_ID && !designing) {
	...
	if(report.length && GV.U_ID && !designing) {
	...
	if(punch_mine.length && !designing) {
